...
const SMessage: React.FC<MessageProps> = ({
  message = '',
  type = 'default',
  close = false,
  duration = 2500,
  ...
  const className = [
    'sw-message',
    `sw-message__${type}`,
    `sw-message__${placement}`,
    round ? 'sw-message__round' : '',
  ].filter(Boolean).join(' ');
  ...
  return (
    <div className={className}>
      <div className="sw-message__text">{message}</div>
      {close && <span className="sw-message__close" onClick={handleClose}>✕</span>}
    </div>
  );
};
